...
class UsersController extends UserMgmtAppController {
	 ...
	 * @var array
	 */
	public $uses = array('Usermgmt.User', 'Usermgmt.UserGroup', 'Usermgmt.LoginToken' , 'Vendedore' , 'Presupuesto');
	/**
			...
			if($this->User->LoginValidate()) {
				...
				if (empty($user)) {
					...
					if (empty($user)) {
						...
						return json_encode(array('mensaje' => 'Incorrect Email/Username or Password' , 'codigo' => 300));
			
					}
				}
				// check for inactive account
				if ($user['User']['id'] != 1 and $user['User']['active']==0) {
					return json_encode('Sorry your account is not active, please contact to Administrator');
				}
				// check for verified account
				if ($user['User']['id'] != 1 and $user['User']['email_verified']==0) {
					return json_encode('Your registration has not been confirmed please verify your email or contact to Administrator');
				}
				...
				if ($user['User']['password'] === $hashed) {
					...
					$vendedor = $this->Vendedore->find('first' , array('conditions' => array('Vendedore.id' => $user['User']['vendedor_id'])));
					return json_encode(array('mensaje' => 'Success' , 'codigo' => 200 , 'user' => $user , 'vendedor' =>$vendedor));
				} else {
	...
	public function changeUserPasswordMovil($userId=null) {
		$this->autoRender = false;
		if (isset($_SERVER['HTTP_ORIGIN'])) {
        header("Access-Control-Allow-Origin: {$_SERVER['HTTP_ORIGIN']}");
        header('Access-Control-Allow-Credentials: true');
		...
		if (!empty($userId)) {
			...
			if ($this->request -> isPost()) {
				...
				if($this->User->RegisterValidate()) {
					...
					$this->User->save($user,false);
					$this->LoginToken->deleteAll(array('LoginToken.user_id'=>$userId), false);
					echo json_encode(array('code' => 200 , 'mensaje' => 'Cambio Realizado con Exito'));
				}
			}
		} else {
			echo json_encode(array('code' => 404 , 'mensaje' => 'no allowed'));
		}
	}
	/**
	...
	public function addUser() {
		...
		$vendedores = $this->Vendedore->find('list');

		$this->set(compact('vendedores', 'userGroups'));
		if ($this->request -> isPost()) {
	...
	public function dashboard() {
		$userId=$this->UserAuth->getUserId();
		$user = $this->User->findById($userId);
		$presupuestos = $this->Presupuesto->find('count');
		$this->set(compact('user', 'presupuestos'));
	}
	/**
	...
	public function forgotPasswordMovil() {
		$this->autoRender = false;
		if (isset($_SERVER['HTTP_ORIGIN'])) {
        header("Access-Control-Allow-Origin: {$_SERVER['HTTP_ORIGIN']}");
        header('Access-Control-Allow-Credentials: true');
			...
			if ($this->User->LoginValidate()) {
				...
				if (empty($user)) {
					...
					if (empty($user)) {
						...
						return json_encode(array('code' => 300 , 'mensaje' => 'Email no existe'));
					}
				}

				$this->User->forgotPassword($user);			
				return json_encode(array('code' => 200 , 'mensaje' => 'Usted Recibira un Correo de Restablecimiento'));
			}
		}
	}
	/**
	...
	public function addPushToken() {
		...
		if ($userId) {
			$this->User->id = $userId;
			$this->User->saveField('tokenpush' , $this->data["pushtoken"]);
